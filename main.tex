...
AR guidance system for laparascopic surgery of the uterus.
One of the major advantages of the method is that it does not require a special hardware but it works with a standard monocular laparoscope and with an off-the-shelf computer.
This enables a quick set-up in the surgery room and no other hardware that may interfere, perturb or distract the work of the surgeon.
Overall, the required steps for calibration, 3D reconstruction and registration take around $\sim \SI{10}{\minute}$.
Most of the interactive and manual operations, such as the masking and the contour annotations are carried out by an operator and can be done in parallel while the surgeon can proceed with other surgery tasks (\eg clamping).
Thus the impact on the clinical activity of the surgeon is reduced and limited to acquiring images for the calibration and the 3D reconstruction.
Concerning the augmented reality and the visual feedback, the experiment shows that the proposed approach for the tracking is robust and responsive.
It runs in real-time and the jittering is usually very low and can be adjusted by adding new keyframes.
One of the limitations of our system is that it still rely on manual interactions that will drive our research directions to further improve the usability and the accuracy of the system.
The masking of the images for the 3D reconstruction still require the user to draw the contour for each image.
Even if the effort is mitigate by the use of a touchscreen that eases the task and reduce the time to around a minute, automatically detecting the organ in the image would further the usability.
We are currently investigating the use of Convolutional Neural Networks to automate the task by training a network that can automatically segment the image.
In that case the user interaction will be limited to validate or remove possible spurious detections.
Similarly, the neural network can also be used to automatically extract the occluding contour fragments needed for the initial registration.
This, again, will significantly improve the required effort.
As for the tracking part, one current limitation is the manual step for adding new keyframe.
An interesting and challeging research direction is to automate this step in order  to account for appearance change during surgery.
This is non-trivial because of the potential for drift caused by the accumulation of small registration errors.
}
