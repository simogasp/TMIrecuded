...
%  \includegraphics[width=0.4\columnwidth]{./figs/phantom.png}
  \includegraphics[width=0.4\columnwidth]{./figs/affineError.pdf}
  \includegraphics[width=0.4\columnwidth]{./figs/mechanicalError.pdf}
...
We computed TRE for each voxel $\vet{q} \in \mathcal{V}$ in each image as $\| f( \Theta(\vet{q}),\mathbf{x}_t) -  \mat{M}_{\text{gt}}(\vet{q}) \|$, where $\vet{x}_t$ are the model registration parameters estimated by our pipeline and $\mat{M}_{\text{gt}}$ is the ground truth pose provided by the MVS reconstruction. 
