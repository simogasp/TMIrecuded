...
%Extended version
\SG{In order to capture sharp images and select keyframes that have a significant spatial displacement between them, we developed a new tool to help the exploration.
We extract SIFT features from a frame and we track them along the frames of the live feed using~\cite{Bouguet2001}.
When the average 2D displacement of the features exceed a threshold (default, $\SI{100}{pixels}$) or too many features are lost \wrt the initial frame (default, \SI{35}{\percent}), we consider that the camera has undergone a sufficient spatial displacement: we are ready to acquire a new keyframe and the UI will ask the surgeon to hold the camera still.
We extract and track new features and the keyframe is acquired when the maximum displacement of the features has been steady below a given threshold (default, $\SI{3}{pixels}$) for the last $M=15$ frames.
Then the process start again until at least $N=15$ keyframes are acquired.}
