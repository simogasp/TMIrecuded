...
Once the initial registration is computed, the tracking stage is performed in real-time from the live feed of the laparoscope. 
Our approach updates the initial registration at each frame and it is robust to common challenges including occlusions from \eg, surgical tools, partial views and viewpoint changes.
...
Let $I^*$ the index of the keyframe with the largest amount of candidate matches.
...
From the set of candidate matches, we find the best pose hypothesis that explains these  matches using the robust RANSAC~\cite{Fischler:1981:RSC:358669.358692} framework, relying on the OpenCV's default implementation.
In some images tracking may be impossible or unreliable, if the organ is not visible or only a small part is visible in the image. A good indicator for deciding if pose can be estimated reliably is the number $n_c$ of inlier matches found by RANSAC. If this is below a threshold (default of $8$), we reject the pose and consider the organ to be untrackable in that image. Finally, to reduce jitter artefacts, we feed the pose estimates into an Extended Kalman Filter (EKF). 
...
This helps to mitigate any remaining jittering and stabilizes tracking if the organ is viewed from a viewpoint that is very dissimilar to the ones present in the keyframe set.
\SG{New keyframes can also help to stabilize the jittering when relatively small changes happens between the exploratory  and the tracking phase, due, \eg, to changes in appearance (bleeding spots, change of texture because of the clamping) and in shape (manipulation of the organ in between the two phases).}
...
We perform this by adding the features and their associated 3D points of the current image to $\mathcal{F}$.
We currently refrain from automating the process of adding new keyframes because without care it can lead to drift.
Specifically, errors in the estimate pose then leads to incorrectly aligned features in $\mathcal{F}$, leading to a degradation in tracking accuracy in later frames.
\SG{The operator can add the current frame as a new keyframe by pressing a bottom on the interface: a still image of the frame with the augmented reality superposed is shown to the operator for validation.
If the augmentation is correctly aligned \wrt to the image then the operator validates and the new keyframe is added, otherwise it is discarded.}
