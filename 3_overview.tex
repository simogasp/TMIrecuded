...
To provide real-time AR, only the tracking stage needs to be real-time. To minimize workflow interruption, we require the initial registration to be computed in no longer than a few minutes. With our current implementation this takes approximately three minutes \TC{[Is this not lower now you have a c++ implementation?]} on a standard workstation PC (approximately two minutes for manual pre-processing and one minute for optimisation). 
