...
%  \caption{From left to right, the phantom used for the experiment, the TRE for the affine deformations and the mechanical deformations and the distribution of TRE on the surface of the 3D model.}
  \caption{From left to right, the TRE for the affine deformations and the mechanical deformations. The deformation extent increases along the $x$-axis. For each deformation the TRE is plotted for different depth distances from the head of the uterus.}
  \label{fig:TRE}
\end{figure}

\SGG{In our previous paper~\cite{Collins2044} we presented a thorough evaluation of the initial registration step on a 3D printed uterus phantom and different synthetically deformed shapes of the CAD model.
...
We collected $\termName{N}{gt}=200$ images of the phantom and we perform a MVS metric reconstruction with the help of a marker of known size.
...
For each generated deformation $\Theta(\cdot)$ we performed our non-rigid registration with the MVS model and we ran our tracker on the remaining $\termName{N}{gt}-\termName{N}{s}$ images.
We discretized the GT model volume with a grid of $100\times100\times100$ voxels and, denoting $\mathcal{V}$ the set of voxels in the interior of the uterus, we can define the registration error for each grid point $\vet{q} \in \mathcal{V}$ for a given tracked image as $\| f( \Theta(\vet{q}),\mathbf{x}_t) -  \mat{P}_{\text{gt}}(\vet{q}) \|$, where $\vet{x}_t$ are the model registration parameters estimated by our pipeline (initial registration and tracking) and $\mat{P}_{\text{gt}}$ is the ground truth pose provided by the initial MVS reconstruction.
\fig{fig:TRE} shows the TRE error for each type of deformation at different depth values from the uterus head as the average error over all the tracked frames.
...
The error increases towards the cervix, from $\SI{8.2}{\milli\metre}$ to $\SI{35}{\milli\metre}$.
}
