...
This is non-trivial because of the potential for drift caused by the accumulation of small registration errors. Finally, we have quantitatively evaluated \textit{in-vivo} tracking accuracy, but more work is required to evaluate Target Registration Error (TRE) of sub-surface structures.
This is extremely hard to achieve \textit{in-vivo} with human data, and it has not been done in related AR systems with other deformable organs.
We aim to perform this in a follow-up clinical trial using a hybrid OR with interventional CT imaging \SGG{following the ideas proposed in~\cite{Bernhardt2016}: the laparoscope pose can be estimated \wrt the intraoperative 3D volume if its tip is in the CT acquisition field of view thus enabling a thorough assessment of the tracking accuracy}.
}
