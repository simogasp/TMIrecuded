...
\SG{We require two interfaces to the deformation model. 
The first is the \emph{transform function} $f(\vet{p};\vet{x}_t): \Omega\rightarrow \mathbb{R}^3$: given the the model's parameters $\vet{x}_t$ at time $t$, it transforms a 3D point $\vet{p}$ of the model's 3D domain $\Omega\subset \mathbb{R}^3$ to the target coordinate frame (in our case, the laparoscope coordinates frame). 
The second interface is the internal energy function $\Einternal(\vet{x}_{t}):\mathbb{R}^{d}\rightarrow\mathbb{R}^{+}$, with $d$ the dimensionality of $\vet{x}_{t}$.
Given the model's parameters $\vet{x}_t$ at time $t$, it returns the internal energy for transforming the organ according to  $\vet{x}_t$.}
...
\SG{The initial registration is important for two main reasons. Firstly, the different patient posture between pre-operative and intra-operative steps and the insufflation induce a significant deformation of the organ. Initial registration estimates such change of shape from the intra-operative state, or \emph{reference state}, and the intra-operative state. 
Secondly, during the tracking stage we assume that the organ does not undergo significant deformations, so  that the tracking stage can be reasonably modelled with a rigid motion. Having the two states registered allows us to associate texture with the organ's surface, which eases the complexity of the tracking.}
...
\SG{In practice, the rigidity assumption during tracking is reasonable because during the period of live AR guidance the surgeon does not deform the organ in a significant way.}
...
\SG{The manual pre-processing takes on average $\sim\SI{2}{\minute}$ while the optimisation takes on average $\sim\SI{1}{\minute}$.}
