...
This ambiguity is always present using motion-based methods, including SfM and SLAM. %This is a problem because without knowing the reconstruction's absolute scale, we cannot correctly register the model. 
...
We (\emph{\textrm{i}}) ease the contour marking process by using interactive touchscreen requiring only non-precise finger strokes with a touchscreen interface, thus making the approach much faster and more practical in the OR.
...
We (\emph{\textrm{iii}}) introduce for the first time a semi-automatic mechanism to collect the frames for the 3D reconstruction of the organ by SfM.
...
We show that this enhancement has an important impact on reconstructed model quality and tracking robustness.
We (\emph{\textrm{iv}}) considerably improve the tracking algorithm to work with a robust real-time SIFT detector which sensibly increases the robustness and smoothness of tracking.
We present a manual mechanism to update the model's features over time to overcome small appearance and geometric changes in the organ during the tracking step.
