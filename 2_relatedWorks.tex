...
\SG{Recently, Convolutional Neural Networks (CNNs) have been shown to recover depth information endoscopic images (colonscopy~\cite{Liu2018} and bronchoscopy~\cite{Visentini2017}), however they have been completely unsuccessful with laparoscopic images because of the large variability in image content.}
...
Of the stereo methods, some perform registration with a manual GUI~\cite{Cohen2010Prostate,haouchine13} and others perform it semi-automatically with manually located landmarks~\cite{21142942,conf/miccai/Amir-KhaliliNPHA13,hamarneh2014igrs,Su2009} \SG{or using 3D features detected on the surface~\cite{Reichard2017}}.
...
\SG{Recent works have proposed to use learning techniques to estimate the deformations of the organs~\cite{Brunet2019,Pfeiffer2019,Mahmood2018}.
Even if the preliminary results on synthetic data are promising they still need to be proven to work in real case scenario.}
...
Markerless tracking can also be addressed using visual Simultaneous Localisation and Mapping (SLAM)~\cite{Thrun2002Robotic,Mahmoud2017}. 
...
Specifically four main contributions considerably improved the previous work.
We (\emph{\textrm{i}}) ease the contour marking process by using interactive touchscreen requiring only non-precise finger strokes with a touchscreen interface, thus making the approach much faster and more practical in the OR setting.
We (\emph{\textrm{ii}}) generalize our approach to work with all deformable models of interest, including mechanical models such as Finite Element Models (FEMs).
We (\emph{\textrm{iii}}) introduce for the first time a semi-automatic mechanism to collect the frames for the 3D reconstruction of the organ.
...
We show that this enhancement has an important impact on the reconstruction model and it also benefits the quality of tracking.
We (\emph{\textrm{iv}}) considerably improved the tracking algorithm to work with a robust real-time SIFT feature detector which sensibly increase the robustness and the smoothness of the tracking.
We present a manual mechanism to update the model's features over time to overcome small changes in the appearance and the geometry of the organ between the exploration and tracking step.
This allows us to handle gradual texture changes of the organ during surgery and significantly improves tracking quality as well.
We compare tracking performance against the state-of-the-art in real-time monocular laparoscopic tracking (ORB-SLAM~\cite{orbslam_laparo}) and outperform it by a wide margin.}

The result is a system that we have tested several times live in the OR, which we recall has not been previously achieved for any organ, including the uterus, using monocular laparoscopes and a markerless approach.
