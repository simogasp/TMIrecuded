...
\SG{\fig{fig:initialRegOverview} shows the initial registration problem, which is more challenging to solve than the tracking stage because of two main issues: \textit{(i)}  no texture information is available for the organ's surface and \textit{(ii)} the registration is non-rigid.}
...
\SG{We sample the video into $N$ intervals (in practice, $N=10$) and we extract $N$ keyframes $\{K_1,\dots K_N\}$ (see \fig{fig:cannula}), one for each $i\in [1,N]$. 
...
\SG{We then run a state-of-the-art dense SfM\&MVS open source library~\cite{AliceVision}. 
...
Since any MVS reconstruction can be only computed up to an unknown global scale factor $s$, we solve for $s$ jointly with registration process.}
...
\SGC{BEFORE IT WAS JUST $\mat{M}$, BUT I THINK IT'S  $\mat{M}_a$, ISN'IT?}

\SG{Initially, \xref is initialized with a a rigid transform $\mat{M}_a\in \mathcal{SE}_4$.
If the laparoscope is in a canonical position \wrt the organ, $\mat{M}_a$ can be considered known \emph{a priori}.
Otherwise, we devised an interactive and relatively simple procedure to provide a rough estimate of \mat{M}.
The method is based on solving the PnP problem~\cite{Lepetit2009Epnp} between 3D points on the organ's surface model and their corresponding 2D points on one of the keyframe images (\wlg, say $K_1$).
The operator can freely rotate the 3D model to present it from a similar viewpoint as $K_1$.
Then, using an interactive user interface, she selects at least $4$ 3D-2D point correspondences between the 3D model and the image.
...
\SG{In order to provide a first initialization of the reconstruction scale factor $s$, we apply $\mat{M}_a$ to the model and we use OpenGL to render a synthetic image of it.
Note that for the rendering we use the intrinsic parameters of the laparoscope estimated during the calibration.
...
\SG{More precisely, for any 3D point $\vet{q}_j$, $s$ can be estimated as $s\approx d(x_j,y_j)/\tilde{d}_j$, where $\tilde{d}_j$ is the depth of $\vet{q}_j$, and $(x_j,y_j)$ its corresponding 2D point in the image of $K_1$.
For a more robust estimate of $s$, we consider the median value using all points $s=\mathrm{median} \{d(x_j,y_j)/\tilde{d}_j\}$. 
}
...
\SG{Besides the model's internal energy $\Einternal(\vet{x})$, the energy function $E(\vet{x},s)\in\mathbb{R}^+$ consists of two other terms.
We introduced a point cloud data term \Epoint to help the organ's surface to fit to the reconstructed point cloud.
In order to constrain the organ's silhouette contours to fit to the silhouette contour fragments we included a contour data term \Econtour. 
...
\SG{For the point cloud data term \Epoint we use an ICP-based energy term: it employs a set of \emph{virtual point correspondences} $\mathcal{P}=\{\vet{p}_1,\dots \vet{p}_M\}$, where $\vet{p}_j\in\partial \Omega$ is the unknown position of point $j$ on the organ's surface mesh $\Omega$. 
...
In fact, from the MVS reconstruction there may be points of $\mathcal{Q}$ that does belong to the organ or that are poorly reconstructed.
...
\SG{In a similar manner, \Econtour employs virtual point correspondences on the organ surface mesh's occluding contours.
More explicitly, for a given pair $(\vet{x},s)$ and a given keyframe $i$, we generate a set of virtual correspondences $\mathcal{R}_i=\{\vet{r}_1,\dots ,\vet{r}_{C(i)}\}$
...
In order to compute the correspondence we first apply $\ensuremath{f(\cdot;\vet{x})}$ 
to the organ's surface mesh, and we bring the model in the reference frame of the camera $i$ using $(\mat{R}_{i}, s\,\vet{t}_{i})$.
Then, using the same intrinsic parameters as the laparoscope, we render the surface mesh and we store all the pixels on the silhouette boundary in a set $\mathcal{B}$. 
...
\SG{In order to improve convergence, we use a stiff-to-flexible strategy to optimize $E$. We start with a stiff model, we optimize $E$, and then we reduce the stiffness to account for more and more deformation.
