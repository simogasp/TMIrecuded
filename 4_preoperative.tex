...
We extract SIFT features from a frame and we track them along the frames of the live feed~\cite{Bouguet2001}.
When the average 2D feature displacement exceed a threshold (default, $\SI{100}{pixels}$) or too many features are lost \wrt the initial frame (default, \SI{35}{\percent}), we consider that the camera has sufficiently moved. We are then ready to acquire a new keyframe and the UI  asked the surgeon to hold the camera still.
We extract and track new features and the keyframe is acquired when the maximum displacement of the features remains steady and below a given threshold (default, $\SI{3}{pixels}$) for the last $M=15$ frames.
...
We then use a touch-screen interface to manually roughly segment the uterus in the keyframes, so that the background is masked and only the organ is reconstructed. This takes a few seconds per keyframe.
...
The method is based on solving the PnP problem~\cite{Lepetit2009Epnp} between 3D points on the organ's surface model and their corresponding 2D points on one of the keyframes, which is selected interactively.
...
Note that the correspondences are not required to be accurate, as this only serves as first, rough initialization of the camera pose.}
...
To provide a first initialization of the reconstruction scale factor $s$, we apply $\mat{M}_a$ to the model.
We then use OpenGL to render a synthetic image of it using the calibrated intrinsic parameters of the laparoscope. 
OpenGL's $z$-buffer provides a depth map $d(x,y)$ from which we can then compute $s$ by comparing depths in $d$ to depths in $\mathcal{Q}$.
...
More precisely, for a 3D point $\vet{q}_j$, an estimate of $s$ is  $s_j=d(x_j,y_j)/\tilde{d}_j$, where $\tilde{d}_j$ is the depth of $\vet{q}_j$, and $(x_j,y_j)$ its corresponding 2D point.
The final robustified estimate is obtained as the median over all points, \ie $s=\mathrm{median} \{s_j\}$. 
...
We introduce a point cloud data term \Epoint to help the organ's surface to fit the reconstructed point cloud.
...
For the point cloud data term \Epoint we use an ICP-based energy term: it uses a set of \emph{virtual point correspondences} $\mathcal{P}=\{\vet{p}_1,\dots, \vet{p}_M\}$, where $\vet{p}_j\in\partial \Omega$ is the unknown position of point $j$ on the organ's surface mesh $\Omega$. 
...
These points should not be aligned and they are taken care of by the M-estimator, reducing their influence on the energy.
We experimented with different types of estimators and found that a pseudo-L1 $\rho({x})\overset{\mathrm{def}}{=}\sqrt{{x}^{2}+\epsilon}$ offers good results (typically, $\epsilon = 10^{-4}$).
...
In a similar manner, \Econtour uses virtual point correspondences on the organ surface mesh's occluding contours.
...
\begin{split}
 ...
 &  \frac{1}{C}\sum_{i=1}^{N}\sum_{\substack{\vet{c}_{k}\in\mathcal{C}_{i}\\\vet{r}_{k}\in\mathcal{R}_{i}}}\rho\left(\|\pi\left(f(\vet{r}_{k})\right)-\vet{c}_{k}\|\right),
\end{split}
\end{equation}
