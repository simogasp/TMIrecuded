...
We first present results with offline videos of real surgery data, where we compare our new tracking algorithm \wrt our previous work and SLAM, then describe our experiences with deploying and running the pipeline in the real clinical setting.
...
We finally evaluate three specific points of our pipeline: our method of automatic keyframe selection, occlusion resistance in tracking and the use of additional keyframes created during tracking.
Additional video material is available here: \url{https://bit.ly/2m9rGHH}.
...
\label{sec:orbslam}
We present results on recorded video captured during laparoscopic surgery. We test accuracy, computational complexity and the influence of different feature detectors in our real-time organ tracking by detection system, named RT-OTD in the experiments, and we compare it with ORB-SLAM, which is state-of-the-art for SLAM-based tracking in laparoscopic videos~\cite{orbslam_laparo}. We also show augmentation results, completing the AR pipeline.
...
Each video includes around $500$ frames showing image motion of the uterus due to motion induced with the cannula and camera motion. The uterus intraoperative surface is obtained with an exploratory video with $15$ keyframes. 
...
We tested our tracking method RT-OTD using a real-time SIFT implementation~\cite{Griwodz2018Popsift} and the SURF-GPU OpenCV's implementation on two videos.  
\begin{table}[]
\centering
...
\vspace{-2mm}
\end{table}
We see from \tab{tab:matches} that RT-OTD using SIFT is able to establish more camera poses (\SI{99.5}{\percent} and \SI{100}{\percent} of the frames) compared with SURF features (\SI{91.2}{\percent} and \SI{89.6}{\percent}, respectively).
...
\caption{Columns 1 and 2 show the estimated translation and rotation using SURF and SIFT for a $5000$ frame video.}
...
We show in \fig{fig:SurfVsSift} the pose components over the $5000$ frame video. This  shows that SIFT provides a more stable estimate for both pose component, \ie there are much fewer spikes in the estimates, which are a typical sign of incorrect pose estimates. This translates to a more stable motion estimate, improving overall AR quality with SIFT.
...
\fig{fig:myomas}(b) shows the successful alignment between the preoperative data and the MVS reconstruction. The cost function \eq{eq:totalCost} was optimized in $6$ iterations in approximately \SI{21}{\second}. 
...
Finally, \fig{fig:myomas}(d) reports the visual augmentation of the myoma in some frames of a video recorded during the surgery, clearly showing that the uterus is tracked well over the video. 
The full video is provided as supplemental material along with other videos from other patients.
...
\subsection{Use with Patients in the Operating Room}
We tested our AR system in laparoscopic surgery~\cite{bourdel2017} with three patients with one, two and three uterine myomas respectively. 
For each patient, we built the 3D preoperative models of the organ and the myomas from preoperative T2-weighted MR.
% Three-dimensional models of the patient uteri and myomas were constructed before surgery from T2-weighted MR. 
We used our system to augment the laparoscopic video so that the surgeon could see the location of the myomas in real time. 
...
Overall, we show that automatic keyframe selection improves the quality of reconstruction and  tracking.
Any sampling method is always potentially affected by motion blur whereas guiding the acquisition of the keyframe reduces the risk.}
...
\SG{We evaluated the robustness of tracking \wrt occlusions.
To this end, we performed tests using the keyframes of $5$ patients.
We used the keyframes because the GT pose is readily available from SfM reconstruction.
We considered two types of experiments in which we simulated occlusions of the uterus.
In the first experiment, we used the mask of the uterus to add a black occluder starting from the external contour and towards the center of the uterus.
We gradually increased the size of the occluder and launched tracking on the keyframe.
\fig{fig:stressTest} shows the overall percentage of frames that could be tracked against the occlusion ratio.
The occlusion ratio is computed from the number of pixels of the occluder and of the uterus.
The graph shows that tracking copes with occlusion up to $\SI{60}{\percent}$ occlusion, where $\sim\SI{80}{\percent}$ of the frames are tracked.
For the same experiment, \fig{fig:stressTest} shows the box plot for pose rotational error.
The error is below $\sim\ang{1}$ for up to $\SI{50}{\percent}$ occlusion.}

\SG{In the second experiment we simulated the occlusions as growing black spots randomly distributed on the the uterus.
This can simulate, \eg, the presence of tools or bleeding covering or changing the appearance of the uterus.
\fig{fig:stressTest} shows that tracking successfully estimates pose for up to $\SI{60}{\percent}$ occlusion, where $\sim\SI{90}{\percent}$ of the frames are tracked, with rotational error below or of the order of $\sim\ang{1}$.}
...
\subsection{Additional Keyframes During Tracking}
