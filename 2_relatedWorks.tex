...
This paper is a distillation of contributions we made in three distinct workshop papers ~\cite{Collins2044,Collins2013,Collins2017System}. Within this work four significant landmark results in laparoscopic AR guidance were achieved in the broader context of laparoscopic AR. Firstly, we show for the first time that dense \textit{invivo} 3D reconstruction of a laparoscopic scene is achievable with a monocular laparoscope with structure-from-motion (SfM) and multi-view stereo (MVS). Following this, SfM and MVS have bene applied by various other groups groups for related problems. This reconstruction is provided up to an unknown scale factor, which is the result of an ambiguity between a smaller scene viewed up close with smaller camera motion, compared to a larger scene viewed further away with larger camera motion. This ambiguity is always present using motion-based methods, including SfM and SLAM. This is a problem because without knowing the reconstruction's absolute scale, we cannot correctly register the model. Secondly, we are the first to show is possible to recover the reconstructed scene's absolute scale, by solving it at the same time as solving the initial registration. Specifically, scale is constrained by physical properties of the organ's deformable model, and the problem is solved by jointly registering the model while simultaneously optimizing the reconstruction scale factor. Thirdly, we are the first to show that the organ's silhouette (i.e. contours in the laparoscopic images corresponding to the organ's boundary) can be used to significantly improve the initial registration. This is because the contours complement information from the scene reconstruction, by providing landmarks that fixate the model's at its boundaries. This is particularly important for organs that are very smooth and lack in strong geometric detail, such as the uterus. In these the model can incorrectly slide over the reconstruction and drift from the correct solution. Fourthly, we show for the first how to track a mobile organ (one that can move independently of surrounding structures such as the uterus) using robust feature-based matching within a tracking-by-detection framework. This allows the organ to be tracked over long durations (i.e. several minutes) in the presence of difficulties including partial views, occlusions and when the organ moves out of the laparoscope's field-of-view. 

We have further made several technical improvements designed to improve the translation of our complete AR pipeline towards  
