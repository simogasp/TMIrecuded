...
E  % will default to the driver specified in the system graphics.cfg if no
  % driver is specified.
...
\thanks{T. Collins is with \todo{~} D. Pizarro is with \todo{~} S. Gasparini, N. Bourdel, L. Calvet and A. Bartoli are with EnCoV, IP, UMR 6602 CNRS, Universit\'{e} Clermont Auvergne, Clermont-Ferrand, France. S. Gasparini is also with University of Toulouse, Toulouse INP -- IRIT, France.  N. Bourdel, P. Chauvet, M. Canis are with Department of Gynecological Surgery, Clermont-Ferrand, France.}% <-this % stops a space
}
...
\begin{abstract}
A major research objective in Computer Assisted Intervention (CAI) is to provide guidance systems to aid surgical teams in laparoscopic surgery using Augmented Reality (AR).
This involves registering data from other modalities such as MR or CT, and fusing it with the laparoscopic video in real-time.
...
These problems are challenging for the uterus because it is a weakly-textured, highly mobile organ that moves independently of surrounding structures. In the broader context, this is the first system that has successfully performed markerless real-time registration and AR of a mobile human organ with monocular laparoscopes in the OR. %We present in-silica quantitative results and in-vivo qualitative results, and its use in two he system has also been used in two pre-clinical cases. 
\end{abstract}
