...
We first present results with offline videos of real surgery, where we compare our new tracking algorithm \wrt our previous work and SLAM, then describe our experiences with deploying and running the pipeline in the real clinical setting.
...
\label{sec:orbslam}
We present results on videos recorded during laparoscopic surgery. We test accuracy, computational complexity and the influence of the keypoint detector in our real-time organ tracking by detection system, named RT-OTD in the experiments.
We compare it with ORB-SLAM, which is state-of-the-art for SLAM-based tracking in laparoscopic videos~\cite{orbslam_laparo}. We also show augmentation results, completing the AR pipeline.
...
\caption{Comparing SURF and SIFT on a $5000$ frame video.}
...
\SG{We evaluated the robustness of tracking \wrt occlusions, using the keyframes of $5$ patients.
