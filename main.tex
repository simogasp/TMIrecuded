...
This will drive our research directions to further improve the usability and the accuracy of the system.
First is masking of the keyframes for 3D reconstruction.
Even if the effort is mitigate by the use of a touchscreen that reduces the overall time to around a minute, automatically detecting the organ in the image would further the usability.
We are investigating the use of CNNs to automate the task.
In that case the user interaction will be limited to validate the segmentation.
Similarly, the CNN can be used to automatically extract the occluding contour fragments needed for the initial registration.
As for the tracking part, one current limitation is the manual addition of keyframes.
An interesting and challenging research direction is to automate this step.
